...
describe("register new user", () => {
  ...
  it("no number in password", async() => {
    const data = createRegisterData(validUserData.username,"password@@@@@","password@@@@@");
    await testEndpoint(data, 400)
  });

  it("no special character in password", async() => {
    const data = createRegisterData(validUserData.username,takenUserData.email,"password12345","password12345");
    await testEndpoint(data, 400)
  });

  it("password too small", async() => {
    const data = createRegisterData(validUserData.username,takenUserData.email,"small","small");
    await testEndpoint(data, 400)
  });

  it("username too small", async() => {
    const data = createRegisterData("xx",takenUserData.email,validUserData.password,validUserData.confirmPassword);
    await testEndpoint(data, 400)
  });

  it("username not entered", async() => {
    const data = createRegisterData(inputNotEntered,takenUserData.email,validUserData.password,validUserData.confirmPassword);
    await testEndpoint(data, 400)
  });
  ...
  it("email not entered", async() => {
    ...
    const data = createRegisterData(validUserData.username,takenUserData.email,inputNotEntered,validUserData.confirmPassword);
    await testEndpoint(data, 400)
  });

  it("confirm not entered", async() => {
    const data = createRegisterData(validUserData.username,takenUserData.email,validUserData.password,inputNotEntered);
    await testEndpoint(data, 400)
  });
});
